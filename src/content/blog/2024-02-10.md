...
### Implementation
Notice that the `'*'` is like a "skip".
If $p[j_x + 1:]$ fails to match with with $s[i_x:]$, it should try matching $p[j_x+1:]$ with $s[i_x+1:]$ (assuming $j_x$ exists).

The following code has an extra loop to account for trailing `'*'`.
```java
class Solution {
    public boolean isMatch(String s, String p) {
